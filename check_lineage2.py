...
print(f"Всего колонок: {len(metadata.columns)}")
for i, col in enumerate(metadata.columns):
    print(f"{i+1}. {col.column_name} (full: {col.full_name}) table: {col.table} calc: {col.is_calculation} deps: {col.dependencies}")

print("\n--- Проверка вычисляемых колонок ---")
for col in metadata.columns:
    if col.is_calculation:
        print(f"Вычисляемая: {col.column_name}, тип: {col.calculation_type}, зависимости: {col.dependencies}")

print("\n--- Проверка алиасов SELECT ---")
# Посмотрим на sample_columns_output
